...
def helper(dis):
    ...
    # Debug: Print the disease being searched
    print(f"Searching for disease: '{dis}'")
    ...
    if not med.empty:
        ...
        if len(med) > 0 and isinstance(med[0], str) and med[0].startswith('['):
            try:
                med = ast.literal_eval(med[0])
            except:
                med = [med[0]]  # fallback if parsing fails
    else:
    ...
    if not die.empty:
        ...
        # Convert string representation of list to actual list
        if len(die) > 0 and isinstance(die[0], str) and die[0].startswith('['):
            try:
                die = ast.literal_eval(die[0])
            except:
                die = [die[0]]  # fallback if parsing fails
    else:
    ...
    print(f"Found {len(wrkout)} workout recommendations")
...
def home():
    if request.method == 'POST':
        ...
        if symptoms =="Symptoms":
            ...
            # Clean the predicted disease name to handle any inconsistencies
